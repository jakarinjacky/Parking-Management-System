...
(async()=>{
 ...
 try {
  ...
  await page.locator('#modal [name=memberDiscountPercent]').fill('50');
  await page.locator('#modal [name=roomQuota]').fill('2');
  await page.locator('#modal button[type=submit]').click();
  await page.locator('#modal').waitFor({state:'hidden'});
  await page.locator('[data-nav=members]').click();
  assert((await page.locator('#main').innerText()).includes('50%'));
  await page.locator('[data-nav=editor]').click();
  await page.locator('[data-action=roadCurve]').click();
  await page.locator('#modal [name=label]').fill('Test curved road');
  await page.locator('#modal button[type=submit]').click();
  await page.locator('#modal').waitFor({state:'hidden'});
  assert.equal(await page.locator('svg[aria-label="แบบร่างถนนโค้ง"] path').count(),2);
