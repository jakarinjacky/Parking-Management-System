...
อัปเดตรอบ Business / Operations: ดู [PRODUCT_OPERATIONS_TH.md](PRODUCT_OPERATIONS_TH.md)
สำหรับกฎราคา สมาชิก Gateway bench, transaction, HTTPS และ backup/restore ที่เพิ่มล่าสุด
ส่วนที่ต้องใช้โดเมน/บัญชีร้านค้า/ฮาร์ดแวร์จริงยังไม่ถือว่าเปิดใช้งานแล้ว
...
- สมาชิกมีช่วงสิทธิ์ ทะเบียน ห้อง/หน่วยงาน ส่วนลดอัตโนมัติและโควตาต่อห้องเมื่อกำหนด policy
...
- Gateway อุปกรณ์เปิดแยกหลัง HTTPS ใช้คีย์รายอุปกรณ์และ heartbeat จริง; LED bench เท่านั้น ไม่มีคำสั่งไม้กั้น
...
1. มี PostgreSQL JSONB transaction ข้าม process แล้ว; ยังไม่แยก schema/ย้ายลานเดิม และ Session ต้อง sticky routing
2. มีส่วนลด/โควตา visitor approval คูปองตรวจด้วยคนและ Valet; ยังไม่เชื่อม POS/PMS หรือแอปอนุมัติของลูกบ้าน
3. มีราคาประเภทรถ เสาร์อาทิตย์ วันหยุด เพดานต่อ 24 ชั่วโมง และส่วนลด; ยังไม่มีค่า EV ตามพลังงาน
4. มีถนนโค้งแบบร่างหน่วยเมตรแยกจาก routing; ยังไม่มีรัศมีเลี้ยว ทางลาด หรือแบบก่อสร้างรับรอง
5. มี Gateway heartbeat/LED bench และตัวอย่าง ESP32; ยังไม่ได้ทดสอบอุปกรณ์ของลูกค้า ไม่มี ANPR/คำสั่งไม้กั้น/fail-safe จริง
...
9. มี backup timer, isolated restore drill และ scheduled retention; ยังต้องกู้ backup จริง ทำนอกเครื่องและ alert ปฏิบัติการ
